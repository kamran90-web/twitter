<div class = "flex p-4 border-b border-b-gray-400"> 
    ...
    <a href = "{{$tweet->user->path()}}">
        <a href = "">
        <img src = "{{asset('/storage/'. $tweet->user->avatar)}}" alt = "" class = "rounded-full mr-2" style = "width:50px;height:50px;">
    </a>
    </div>   
    ...
    <div>
       ...
       <p class = "text-sm mb-3"> {{$tweet->body}}<p>
        {{-- created {{$tweet->user->created_at->diffForHumans() }} --}}
        <div class = "flex">
            <form method = "POST" action = "/tweets/{{$tweet->id}}/like">
                @csrf
                <div class = "flex items-center mr-6 ">
						<svg viewBox="0 0 20 20" class = "text-gray-500 mr-1 w-5 {{$tweet->likes ?'text-blue-500' : 'text-gray-500'}}">
                        ...
                        <button type = "submit">{{$tweet->likes ?: 0}}</button>
                </div>
            </form>
            ...
            <form method = "POST" action = "/tweets/{{$tweet->id}}/like">
                ...
                <div class = "flex items-center">
						<svg viewBox="0 0 20 20" class = "text-gray-500 mr-1 w-5 {{$tweet->dislikes?'text-blue-500' : 'text-gray-500'}}">
						<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
							<g class = "fill-current">
                        ...
                        <button type = "submit" class = "">{{$tweet->dislikes ?: 0}}</button>
                </div>
            </form>    
        </div>
    </div>
